...
The TUI uses Python's standard `curses` module, so it needs no web server or network port. The visible screen is intentionally minimal: titles, session controls, governance panels, and footer hints remain hidden. After sending, only the latest message is shown above the bottom question rectangle with the `agent` prefix, for example `agent iraq`; a compact metadata line shows `Model`, `Repo`, and `Used`. Full message history and governance data continue to be stored internally in SQLite.
